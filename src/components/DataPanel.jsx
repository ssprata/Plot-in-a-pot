...
export default function DataPanel({
  ...
  validationResult, translations, setTranslations
}) {
      ...
      <div className={`flex flex-col h-full p-4 transition-opacity duration-200 ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 p-2 text-[10px] font-black uppercase border-2 transition-colors 
      ${activeView === 'validation'
                ? 'bg-gray-900 text-white dark:bg-gray-100 dark:text-gray-900 border-gray-900'
                : 'bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-400 border-gray-300 dark:border-gray-600'
              }`}
          >
        ...
        <div className="flex-1 overflow-y-auto">
          {activeView === 'validation' ? (
            <>
              ...
              {/* --- EXIBIÇÕES DE ESTADO (Parser/Validação) --- */}
            ...
            <TranslationTable translations={translations} setTranslations={setTranslations} />
          )}
        </div>
