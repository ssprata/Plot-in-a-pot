...
export default function VariablesModal({
  ...
  // Text dictionary for PT and EN support
  const txt = useMemo(() => ({
    title: isPt ? 'Variáveis do Jogo' : 'Story Variables',
    globalScope: isPt ? 'Variáveis Globais' : 'Global Variables',
    localScope: isPt ? 'Cena Atual (Modificações)' : 'Current Scene (Local)',
    searchPlaceholder: isPt ? 'Procurar variáveis...' : 'Search variables...',
    createBtn: isPt ? 'Criar variável' : 'Create variable',
    colName: isPt ? 'Nome' : 'Name',
    colType: isPt ? 'Tipo' : 'Type',
    colValue: isPt ? 'Valor Inicial' : 'Default Value',
    colLocalValue: isPt ? 'Valor na Cena' : 'Scene Value',
    activeInScene: isPt ? 'Ativo na Cena' : 'Active in Scene',
    renameHint: isPt 
      ? 'A renomeação propaga-se automaticamente por todo o projeto.' 
      : 'Renaming propagates automatically across the entire project.',
    noVars: isPt ? 'Nenhuma variável encontrada.' : 'No variables found.',
    noVarsGlobalDesc: isPt 
      ? 'Cria variáveis globais usando o botão "+ Criar variável" no topo.' 
      : 'Create global variables using the "+ Create variable" button above.',
    noVarsLocalDesc: isPt 
      ? 'Ativa variáveis globais na cena atual para as modificar localmente.' 
      : 'Activate global variables in the current scene to modify them locally.',
    confirmDeleteTitle: isPt ? 'Apagar Variável' : 'Delete Variable',
    typeNumber: isPt ? 'Número' : 'Number',
    typeString: isPt ? 'Texto' : 'String',
    typeBoolean: isPt ? 'Booleano' : 'Boolean',
    delete: isPt ? 'Apagar' : 'Delete',
    cancel: isPt ? 'Cancelar' : 'Cancel',
    close: isPt ? 'Fechar' : 'Close',
    allTypes: isPt ? 'Todas as variáveis' : 'All variables',
    sceneContext: isPt ? 'Cena:' : 'Scene:'
  }), [isPt]);
  ...
  const typeStyles = {
    boolean: {
      label: 'bool',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 mr-1 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      ),
      badge: 'text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/40 border-purple-200 dark:border-purple-900/50'
    },
    number: {
      label: 'num',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 mr-1 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
        </svg>
      ),
      badge: 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 border-blue-200 dark:border-blue-900/50'
    },
    string: {
      label: 'str',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 mr-1 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h7" />
        </svg>
      ),
      badge: 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-900/50'
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-xs">
      {/* Modal Card */}
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-2xl flex flex-col w-[850px] h-[580px] overflow-hidden transition-all text-sm animate-fade-in-down">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/30 rounded-lg text-blue-600 dark:text-blue-400">
            ...
            <div>
              <h2 className="font-bold text-gray-900 dark:text-gray-100 text-base">{txt.title}</h2>
              {selectedNode && (
                <div className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500 mt-0.5 font-medium">
                  <span>{txt.sceneContext}</span>
                  <span className="bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded text-gray-600 dark:text-gray-300 border border-gray-200/50 dark:border-gray-700/50 font-mono">
          ...
          <button
            onClick={onClose}
            className="p-1.5 border border-gray-200 dark:border-gray-800 rounded-lg bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors shadow-xs"
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* Sidebar */}
          <div className="w-56 border-r border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 p-4 flex flex-col gap-5 select-none">
            ...
            <div className="flex flex-col gap-1">
              ...
              <button
                ...
                className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-left font-medium transition-all ${
                  activeCollection === 'global'
                    ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border border-blue-100/30 dark:border-blue-900/30'
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-gray-950">
            {/* Toolbar */}
            <div className="flex items-center justify-between px-6 py-3 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 gap-4">
              {/* Search bar */}
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-8 py-1.5 border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
                />
                {searchQuery && (
                  <button
              ...
              {activeCollection === 'global' && (
                <div className="relative shrink-0" ref={createMenuRef}>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-semibold shadow-sm hover:shadow-md transition-all active:translate-y-0.5"
                  >
                  ...
                  {/* Dropdown Menu */}
                  {isCreateMenuOpen && (
                    <div className="absolute right-0 mt-2 w-44 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg shadow-xl py-1 z-50 flex flex-col">
                      <button
                        onClick={() => handleCreateVariable('number')}
                        className="flex items-center gap-2 px-3.5 py-2 hover:bg-gray-50 dark:hover:bg-gray-700 text-left font-medium text-gray-700 dark:text-gray-200 transition-colors"
                      >
                        {typeStyles.number.icon}
                        <span>{txt.typeNumber}</span>
                      </button>
            ...
            <div className="flex-1 overflow-auto px-6 py-4">
              {displayedVars.length > 0 ? (
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800 text-gray-400 dark:text-gray-500 font-bold uppercase text-[10px] tracking-wider select-none">
                      {activeCollection === 'local' && (
                        <th className="py-2.5 px-3 w-16 text-center">{txt.activeInScene}</th>
                      )}
                      <th className="py-2.5 px-3">{txt.colName}</th>
                      <th className="py-2.5 px-3 w-32">{txt.colType}</th>
                  ...
                  <tbody>
                    {displayedVars.map(({ key, value, type, isLocalActive, localValue }) => {
                      ...
                      return (
                        <tr key={key} className="border-b border-gray-100/60 dark:border-gray-800/40 hover:bg-gray-50/50 dark:hover:bg-gray-800/20 group">
                          ...
                          {activeCollection === 'local' && (
                            <td className="py-3 px-3 text-center">
                              <input
                                ...
                                className="h-4 w-4 text-blue-600 border-gray-300 dark:border-gray-700 rounded focus:ring-blue-500/20 accent-blue-600 cursor-pointer"
                              />
                            </td>
                          )}
                          ...
                          <td className="py-3 px-3">
                            <div className="flex items-center gap-1.5 font-mono text-sm">
                              <span className="text-gray-400 dark:text-gray-500 font-bold select-none">$</span>
                              {activeCollection === 'global' ? (
                                <input
                                  type="text"
                                  defaultValue={key}
                                  ...
                                  className="px-2 py-0.5 border border-transparent hover:border-gray-200 dark:hover:border-gray-700 focus:border-blue-500 focus:bg-gray-50 dark:focus:bg-gray-900 bg-transparent rounded font-mono text-sm text-gray-900 dark:text-gray-100 focus:outline-none w-full"
                                />
                              ) : (
                                <span className="text-gray-900 dark:text-gray-100 font-semibold px-2">
                                  {key}
                                </span>
                              )}
                            </div>
                            ...
                            {activeCollection === 'global' ? (
                              <div className="relative">
                                <button
                                  ...
                                  className={`flex items-center px-2 py-1 border rounded-lg font-medium text-xs uppercase cursor-pointer ${style.badge} transition-all`}
                                >
                                  {style.icon}
                                  <span>{style.label}</span>
                                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 ml-1 opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                {activeTypeDropdown === key && (
                                  <div ref={typeDropdownRef} className="absolute left-0 mt-1.5 w-32 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg shadow-lg py-1 z-50 flex flex-col">
                                    {Object.entries(typeStyles).map(([tKey, tVal]) => (
                                      <button
                                        key={tKey}
                                        onClick={() => handleChangeType(key, tKey)}
                                        className="flex items-center gap-2 px-3 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-700 text-left font-medium text-xs text-gray-700 dark:text-gray-200 transition-colors"
                                      >
                                        {tVal.icon}
                                        <span className="uppercase">{tVal.label}</span>
                                      </button>
                                    ))}
                                  </div>
                                )}
                              ...
                              <span className={`inline-flex items-center px-2 py-1 border rounded-lg font-medium text-xs uppercase select-none ${style.badge}`}>
                                {style.icon}
                                <span>{style.label}</span>
                              </span>
                            )}
                            ...
                            <div className="w-full">
                              ...
                              {type === 'boolean' && (
                                <div className="flex items-center gap-2">
                                  <button
                                    ...
                                    className={`px-3 py-1 border font-black text-xs uppercase rounded transition-all cursor-pointer ${
                                      displayValClean.toLowerCase() === 'true'
                                        ? 'bg-purple-600 border-purple-700 text-white shadow-sm'
                                        : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 text-gray-400 hover:border-purple-300 dark:hover:border-purple-800 dark:text-gray-500 disabled:opacity-40'
                                    }`}
                                  >
                                    true
                                  </button>
                              ...
                              {type === 'number' && (
                                <input
                                  ...
                                  className="w-40 px-2.5 py-1 border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white rounded-lg font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:opacity-40"
                                />
                              )}
                              ...
                              {type === 'string' && (
                                <input
                                  ...
                                  className="w-full max-w-xs px-2.5 py-1 border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white rounded-lg font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 disabled:opacity-40"
                                />
                              )}
                            </div>
                          </td>
                          ...
                          {activeCollection === 'global' && (
                            <td className="py-3 px-3 text-center">
                              <button
                                ...
                                className="p-1 border border-transparent hover:border-red-200 dark:hover:border-red-900 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-lg text-gray-400 hover:text-red-500 transition-all cursor-pointer opacity-0 group-hover:opacity-100"
                                title={txt.delete}
                              >
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center select-none animate-fade-in">
                  <div className="p-4 bg-gray-50 dark:bg-gray-900 border border-dashed border-gray-200 dark:border-gray-800 rounded-full text-gray-400 dark:text-gray-500 mb-4">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  </div>
                  <h3 className="font-bold text-gray-800 dark:text-gray-200 text-sm">{txt.noVars}</h3>
            ...
            <div className="flex items-center justify-between px-6 py-3.5 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
              <div className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-blue-500/80 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                onClick={onClose}
                className="px-5 py-1.5 font-bold text-xs uppercase border border-gray-900 dark:border-gray-200 bg-black text-white hover:bg-gray-800 dark:bg-gray-200 dark:text-gray-900 dark:hover:bg-white rounded-lg shadow-sm hover:shadow-md transition-all active:translate-y-0.5"
              >
                {txt.close}
              </button>
            </div>
