...
\exampleinfo{%
Reconheço a utilização integrada da inteligência artificial Gemini (\url{https://gemini.google.com}) em conjunto com o ecossistema e agente autónomo de desenvolvimento Google Antigravity (\url{https://antigravity.google}) como ferramentas de suporte no processo de codificação, desenho arquitetural e automação de testes lógicos da aplicação \textit{Plot in a Pot}. A inteligência artificial operou como assistente no espaço de trabalho local seguro (\textit{sandbox}), executando tarefas de refatorização e análise de dependências sob estrita validação e supervisão humana.
