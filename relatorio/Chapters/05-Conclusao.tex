...
\begin{enumerate}
    \item \textbf{Execução de IA Local no Cliente via \gls{webgpu}:} De momento, o assistente de IA local para conversão de texto livre baseia-se numa \gls{api} REST externa ou na escuta de uma porta local ligada ao Ollama. Uma expansão futura promissora consistiria em correr \glspl{llm} leves diretamente na memória do navegador do utilizador recorrendo a \textit{frameworks} baseadas em \gls{webgpu} (como o WebLLM). Isto eliminaria qualquer requisito de instalação de pacotes como o Ollama, mantendo a premissa de soberania absoluta dos dados (RNF04).
    \item \textbf{Edição Colaborativa em Tempo Real:} Integrar algoritmos de sincronização distribuída baseados em \glspl{crdt} (\textit{Conflict-free Replicated Data Types}), tais como a biblioteca Yjs ou Automerge. Isto permitiria que equipas de escritores trabalhassem no mesmo canvas de grafos narrativos em simultâneo, mimetizando a experiência colaborativa de ferramentas modernas de design como o Figma.
