...
\begin{itemize}
    ...
    \item \texttt{NomeDaPassagem} -- O título identificador único da passagem. Não pode conter os caracteres \texttt{[} ou \texttt{\char`173} e é sensível a maiúsculas/minúsculas (\textit{case-sensitive}).
    \item \texttt{[tags]} -- Lista opcional de palavras-chave separadas por espaço, delimitada por parênteses retos.
    \item \texttt{\char`173metadados\char`175} -- Objeto opcional em formato JSON delimitado por chavetas, contendo propriedades de layout ou de sistema.
\end{itemize}
...
\subsection{Campos de Geometria JSON}
