...
Implementado em React e no ecossistema React Flow, o PiaP destaca-se pela sua estética neo-brutalista de alto contraste visual, concebida para mitigar barreiras de acessibilidade cognitiva e cromática para autores com daltonismo severo (como deuteranopia e protanopia). O sistema integra: (i) um parser síncrono bidirecional para ler e reconstruir topologias Twee 3 em tempo real; (ii) uma matriz de localização baseada em grelha bidimensional com importação e exportação de CSV; (iii) um motor de validação estática suportado por algoritmos de Pesquisa em Largura (BFS) para analisar a acessibilidade de cenas; (iv) um modo de simulação interativo dotado de um agente inteligente autónomo (\textit{Smart Walker}) que utiliza procura por novidade (\textit{Novelty Search}) para percorrer e testar caminhos automaticamente; e (v) arestas com rota curva parametrizada por splines cúbicas de Catmull-Rom.
...
Implemented in React and using the React Flow ecosystem, PiaP stands out for its high-contrast neo-brutalist visual style, specifically designed to mitigate cognitive and chromatic accessibility barriers for authors with severe color blindness (such as deuteranopia and protanopia). The system features: (i) a synchronous bidirectional parser to read and reconstruct Twee 3 topologies in real time; (ii) a translation matrix layout with CSV import and export capabilities; (iii) a static validation engine powered by Breadth-First Search (BFS) algorithms to audit scene reachability; (iv) an interactive simulation mode equipped with an autonomous agent (\textit{Smart Walker}) that leverages novelty search heuristics to automatically test paths; and (v) customizable edge routing parameterized by Catmull-Rom cubic splines.
