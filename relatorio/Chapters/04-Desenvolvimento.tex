...
\begin{description}
    ...
    \item[Fase 2 -- Motor Lógico:] Implementação da biblioteca \texttt{tweeParser.js} para ler e estruturar o formato Twee 3 em memória como uma lista de adjacências bidirecional, e do motor de validação estática assente em travessias BFS.
...
\begin{listing}[!htpb]
\begin{lstlisting}
// Registo de snapshots e funções de controlo em App.jsx
...
\end{lstlisting}
\caption{Mecanismo de clonagem profunda e gestão da pilha de histórico em App.jsx.}
...
\end{lstlisting}
\caption{Refatorização automática de variáveis lógicas baseada em Regex no VariablesModal.jsx.}
...
\end{lstlisting}
\caption{Parser de importação de ficheiros CSV com suporte à norma RFC 4180 em TranslationMatrix.jsx.}
...
\begin{itemize}
    ...
    \item \textbf{Qwen 3 8B (\texttt{qwen3\_8b}):} O resultado ficou \textbf{inutilizável devido a erros de formatação}. O modelo incluiu o seu raciocínio interno (\textit{``Thinking...''}) mesmo ao início da resposta, o que faz com que o nosso importador falhe imediatamente ao tentar ler o ficheiro. Além disso, deixou um bloco JSON incompleto na posição das passagens (escreveu apenas \texttt{\char`173"} sem fechar), o que faz o parser de JSON falhar, e manteve alguns códigos ANSI.
