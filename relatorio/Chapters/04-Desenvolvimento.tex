...
\begin{itemize}
    ...
    \item \textbf{Gemma 2B:} O modelo misturou os cabeçalhos de novos nós dentro do texto de outras passagens. Como o parser da aplicação não suporta nós aninhados, a importação leu isto como texto simples e falhou a criação no canvas.
    \item \textbf{Qwen 2 7B:} O resultado foi problemático porque o modelo incluiu explicações textuais introdutórias em linguagem natural e deixou a estrutura do código incompleta na resposta final.
    \item \textbf{Qwen 2.5 7B:} Apresentou o melhor desempenho na organização do fluxo. No entanto, ainda incluiu marcas de raciocínio intermédio (\textit{"Thinking..."}) no início da resposta e criou alguns nós órfãos que não estavam ligados ao resto da história.
\end{itemize}
...
O grupo de teste foi composto por seis participantes (U1 a U6), caracterizado pelos seguintes perfis representativos:
\begin{itemize}
    \item \textbf{Perfil de Escrita Criativa e Humanidades (U1):} Escritora de 22 anos com formação secundária na área de Línguas e Humanidades e experiência prévia na criação de narrativas, servindo para validar a fluidez do editor e a facilidade de escrita sem necessidade de conhecimentos de programação.
    \item \textbf{Perfil Técnico (Desenvolvimento de Jogos Digitais):}
    \begin{itemize}
        \item \textbf{U2:} Estudante do Mestrado em Desenvolvimento de Jogos Digitais (\textit{Digital Games Development}) da ESTG com experiência consolidada em desenvolvimento de software, permitindo testar a eficiência da manipulação de dados e a clareza da lista de adjacências.
        \item \textbf{U3:} Estudante do Mestrado em Desenvolvimento de Jogos Digitais (\textit{Digital Games Development}) da ESTG em fase inicial de formação técnica, avaliando a clareza do grafo e a lógica estrutural da plataforma.
    \end{itemize}
    \item \textbf{Perfil de Design e Multimédia (U5):} Estudante de Artes com formação secundária na área de Multimédia, focada na avaliação estética do estilo neo-brutalista, na manipulação visual de caminhos e na ergonomia do \textit{canvas} interativo.
    \item \textbf{Perfil de Marketing (Utilizador não técnico - U4):} Estudante da área de Marketing sem qualquer experiência prévia em escrita interativa, lógica de grafos ou estruturas condicionais. Este perfil revelou-se fundamental para identificar os principais pontos de fricção na interface e as barreiras de entrada para utilizadores leigos.
    \item \textbf{Acessibilidade Visual (U6):} Utilizador com daltonismo severo e baixa acuidade visual, permitindo validar de forma prática a eficácia das redundâncias visuais, dos contornos espessos e da sinalização semântica dupla implementada no editor.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textit{Medidas Corretivas Implementadas:} Desenvolveu-se e integrou-se o modo de simulação interativa (\textit{PlayMode}) direto no canvas e o módulo de conversor de IA local assente em LLMs (via Ollama) para a importação rápida de texto.
\end{itemize}

\subsection{Segunda Leva: Protótipo Intermédio (Maio de 2026)}
A segunda leva de testes ocorreu a \textit{18 de maio de 2026} com os utilizadores U2, U3 e U5, incidindo sobre o funcionamento do conversor de IA, o \textit{PlayMode} e a tradução.
...
\begin{itemize}
    ...
    \item \textit{Feedback e Falhas Identificadas:} Os utilizadores apontaram a sobreposição caótica de arestas retas, a ausência de agrupamento temático para cenas (capítulos) e a quebra frequente da simulação ao renomear variáveis lógicas globalmente. Adicionalmente, a importação de ficheiros \gls{csv} quebrava quando o texto continha vírgulas integradas.
    \item \textit{Medidas Corretivas Implementadas:} Implementaram-se as arestas curvas paramétricas baseadas em Splines de Catmull-Rom com \textit{waypoints} em \texttt{EditableEdge.jsx}, as Zonas de agrupamento visual em \texttt{ZoneNode.jsx}, o painel \texttt{VariablesModal.jsx} para refatorização de variáveis e o suporte à norma RFC~4180 no parser de \gls{csv}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Efeito de Aprendizagem (Viés de Familiaridade):} O refinamento iterativo do sistema expôs parte dos participantes a testes em múltiplas levas de desenvolvimento (conforme a Tabela~\ref{tab:participacao-levas}): apenas U5 participou nas três levas; U1 e U4 na primeira e na terceira; U2 e U3 na segunda e na terceira; U6 participou exclusivamente na leva final. O facto de alguns utilizadores já conhecerem o funcionamento básico da aplicação em testes subsequentes pode ter inflacionado as pontuações SUS das levas intermédia e final, camuflando eventuais atritos que utilizadores completamente estreantes poderiam experimentar no produto final.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Dependência de Hardware e VRAM:} O assistente de importação baseado em \glspl{llm} locais (como o Qwen 2.5 7B) exige uma placa gráfica dedicada com capacidade superior a 8\,GB de VRAM para correr localmente via Ollama com latência aceitável. Esta exigência limita a acessibilidade da funcionalidade em computadores portáteis normais ou de gama baixa (RTX 3050 com 4\,GB de VRAM).
