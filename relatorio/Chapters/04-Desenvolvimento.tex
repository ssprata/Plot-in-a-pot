...
\begin{minipage}{\linewidth}
\centering
\small \textit{Nota: Os valores medem o tempo de CPU decorrido desde o início da operação até ao redesenho final da interface gráfica do canvas do React Flow. O desvio padrão ($\sigma$) para as 50 execuções situou-se entre $\pm 0.3\,\text{ms}$ (escala pequena) e $\pm 4.8\,\text{ms}$ (escala grande), confirmando a estabilidade do processamento local.}
\end{minipage}
\end{table}
...
Os resultados desta fase foram altamente positivos, registando-se uma taxa média de conclusão de tarefas de 95,8\%. Apenas um participante (U4, sem perfil técnico) não concluiu a tarefa de validação autónoma dentro do tempo limite, devido a dificuldades em interpretar as mensagens textuais do validador, o que reforçou a necessidade de incluir alertas gráficos diretamente sobre os nós do canvas. No questionário \textit{System Usability Scale} (SUS), a plataforma obteve uma pontuação média global de 84,6 pontos, posicionando o \textit{Plot in a Pot} no nível de usabilidade considerado ``Excelente'' (Grau~A+). Estes valores traduzem uma evolução muito expressiva face às levas anteriores (61,5 e 73,0 pontos) e validam o sucesso do refinamento iterativo do protótipo.
