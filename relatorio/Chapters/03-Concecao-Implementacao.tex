...
O algoritmo funciona convertendo a sequência ordenada de pontos (composta pelo nó de origem, os \textit{waypoints} intermédios e o nó de destino) em segmentos de curvas Bézier Cúbicas nativas do formato SVG. Para cada par de pontos consecutivos $p_i$ e $p_{i+1}$, os respetivos pontos de controlo $cp_1$ e $cp_2$ da curva Bézier são gerados em tempo real recorrendo aos pontos adjacentes $p_{i-1}$ e $p_{i+2}$, de acordo com a formulação matemática:
\[
cp_{1,x} = p_{i,x} + \frac{p_{i+1,x} - p_{i-1,x}}{6}
\]
\[
cp_{1,y} = p_{i,y} + \frac{p_{i+1,y} - p_{i-1,y}}{6}
\]
\[
cp_{2,x} = p_{i+1,x} - \frac{p_{i+2,x} - p_{i,x}}{6}
\]
\[
cp_{2,y} = p_{i+1,y} - \frac{p_{i+2,y} - p_{i,y}}{6}
\]

A divisão constante por $6$ corresponde à parametrização da tensão da spline, garantindo a continuidade de primeira ordem ($C^1$) na junção entre segmentos (suavidade tangencial). Os pontos calculados são de seguida serializados na propriedade \texttt{d} do elemento SVG \texttt{<path>} através do comando Bézier cúbico \texttt{C cp1.x,cp1.y cp2.x,cp2.y p2.x,p2.y}.
