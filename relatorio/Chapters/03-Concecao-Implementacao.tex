...
No \gls{piap}, o estado centralizado da narrativa é mantido na raiz da aplicação (\texttt{App.jsx}) recorrendo a três vetores de dados estruturados em memória local:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Árvore de Sintaxe Abstrata (\gls{ast}):} O interpretador de condicionais analisa blocos lógicos aninhados construindo uma AST simples que reflete a precedência de cláusulas. Esta estrutura é então avaliada em tempo de execução para calcular o valor de verdade das expressões.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Simulação de API:} O servidor expõe uma \gls{api} REST local na porta padrão (tipicamente \texttt{http://localhost:11434}), que simula a interface padrão de chat da OpenAI.
