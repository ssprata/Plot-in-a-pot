...
\begin{itemize}
    ...
    \item \textbf{Compatibilidade com Compiladores Externos (ex: Tweego):} Sendo o formato Twee 3 uma especificação padrão e aberta, qualquer ficheiro exportado pelo \gls{piap} é totalmente compatível com compiladores de linha de comandos externos como o \textit{Tweego} (permitindo gerar ficheiros HTML executáveis autónomos com todos os recursos e estilos do SugarCube). Contudo, importa clarificar que a nossa aplicação não necessita nem executa o \textit{Tweego} internamente; toda a validação e simulação interativa da história são processadas no próprio navegador através do simulador integrado (\textit{PlayMode}), dispensando quaisquer dependências ou passos de compilação adicionais.
