...
\section{Arquitetura e Implementação do Parser Síncrono}
\label{sec:arquitetura-parser-sincrono}

O módulo responsável pela serialização e desserialização de histórias na plataforma é o parser síncrono, implementado em \texttt{tweeParser.js} (na diretoria \texttt{src/utils/}). Este módulo assegura a tradução bidirecional em tempo real entre a representação visual baseada em grafos do React Flow e a representação textual baseada na especificação Twee 3 \citep{Twee3Reference}.

A sincronização ocorre de forma síncrona diretamente na memória do navegador. Qualquer alteração de coordenadas, edição de texto ou criação de novas ligações no canvas é imediatamente refletida na estrutura interna de passagens e arestas, e vice-versa. Por se tratar de uma aplicação que corre inteiramente no lado do cliente (\textit{client-side}), este mapeamento mantém o estado da sessão atualizado na memória da aplicação, sendo que a persistência física dos dados requer que o utilizador exporte explicitamente o ficheiro \texttt{.twee}.

\subsection{Processo de Importação (Desserialização)}
O processo de importação é gerido pela função \texttt{parseTwee3(source)}, que recebe uma cadeia de caracteres de texto simples no formato Twee 3 e reconstrói o grafo em cinco etapas consecutivas:
\begin{enumerate}
    \item \textbf{Divisão Lógica em Blocos:} O parser segmenta a fonte de texto usando a expressão regular \lstinline|^::| (com flag multiline \lstinline|m|) para delimitar e isolar as passagens em blocos independentes.
    
    \item \textbf{Extração de Cabeçalhos e Metadados:} Para cada bloco, a primeira linha (cabeçalho) é analisada para separar o nome da passagem, as etiquetas (delimitadas por \lstinline|[ ]|) e os metadados visuais (delimitados em formato JSON por \lstinline|{ }|). Esta separação é efetuada isolando a cadeia de caracteres a partir do primeiro caracter \lstinline|{| para evitar que expressões regulares falhem na presença de objetos JSON complexos ou aninhados.
    
    \item \textbf{Conversão de Coordenadas de Zonas:} Os metadados visuais extraídos indicam a posição absoluta do nó (\lstinline|position|: \lstinline|X,Y|) e, opcionalmente, o identificador do nó pai (\lstinline|parent|). Quando um nó pertence a uma Zona de Agrupamento Visual (ver subsection \ref{subsec:zonas-agrupamento}), o parser efetua a conversão de coordenadas absolutas para coordenadas locais relativas ao canto superior esquerdo da zona mãe:
    \[
    ...
    \]
    Esta conversão é obrigatória para que o React Flow renderize corretamente a hierarquia espacial e permita arrastar a Zona mãe juntamente com todos os nós filhos.
    
    \item \textbf{Extração de Ligações (Arestas):} As ligações direcionadas entre passagens são extraídas do corpo de texto (\texttt{conteudo}) do nó recorrendo a duas expressões regulares:
    \begin{itemize}
        \item \textbf{Ligações Twine Clássicas:} Processadas pela regex \lstinline|/\[\[(.*?)\]\]/g|. O parser analisa se a ligação contém caracteres de direcionamento como \lstinline/|/, \lstinline/->/ ou \lstinline/<-/ para separar o texto visível da escolha (rótulo) do identificador do nó de destino. Ligações dinâmicas que envolvam expressões matemáticas ou variáveis (p. ex. iniciadas por \lstinline|$|) são ignoradas pela validação estática de modo a evitar caminhos falsos no grafo.
        
        \item \textbf{Ligações de Macros SugarCube:} Detetadas por uma expressão regular que captura a estrutura de links e redirecionamentos:
        \begin{center}
        \lstinline|/<<link\s+"([^"]+)"\s*>>([\s\S]*?)<<\/link>>/g|
        \end{center}
        O parser analisa o bloco interno à procura de comandos como \lstinline|<<goto "Destino">>| para gerar as arestas direcionadas.
    \end{itemize}
    Se o nó de destino de qualquer ligação não for encontrado no mapeamento global de passagens, é gerado um aviso de link quebrado que é apresentado na interface visual (no painel laranja de alertas).
    
    \item \textbf{Layout Automático de Salvaguarda:} Se o ficheiro importado não contiver metadados de coordenadas (uma situação comum quando as histórias são geradas por ferramentas de escrita pura), o parser deteta que todos os nós raiz estão posicionados em \lstinline|(0,0)| e invoca a biblioteca de layout automático Dagre (\texttt{layoutNodesAndEdges}) para organizar os nós hierarquicamente (de cima para baixo) antes da renderização.
\end{enumerate}

\subsection{Processo de Exportação (Serialização)}
O processo de exportação é efetuado pela função \texttt{exportToTwee3(nodes, edges)}, que traduz a estrutura de dados em memória de volta para uma cadeia de caracteres contínua compatível com a especificação Twee 3:
\begin{enumerate}
    \item \textbf{Garantia de Unicidade de Rótulos:} Como o formato Twee 3 utiliza o título da passagem como o identificador único para as transições, o parser verifica a existência de colisões de nomes entre nós. Se forem detetados nomes duplicados, o exportador adiciona um sufixo numérico (p. ex., \lstinline|Nome (2)|) de forma automática.
    
    \item \textbf{Escapes de Segurança:} Para evitar a quebra do formato do ficheiro e problemas de parsing em compiladores externos, o exportador limpa e escapa caracteres reservados:
    \begin{itemize}
        \item \textbf{No Cabeçalho:} A função \texttt{escapeForTweeHeader} escapa barras invertidas, parênteses retos e chavetas (\lstinline|\|, \lstinline|[|, \lstinline|]|, \lstinline|{|, \lstinline|}|) adicionando uma barra invertida antes de cada caractere.
        
        \item \textbf{No Corpo de Texto:} A função \texttt{escapeForTweeText} deteta ocorrências literais de \lstinline|::| no início de qualquer linha e substitui-as por \lstinline|\::|, impedindo que o compilador interprete incorretamente essa linha como o início de uma nova passagem.
    \end{itemize}
    
    \item \textbf{Reconversão para Coordenadas Absolutas:} Para manter a compatibilidade com a especificação Twee 3, as posições relativas dos nós filhos de Zonas são convertidas novamente para coordenadas absolutas no plano cartesiano antes da escrita física no ficheiro:
    \[
    ...
    \item \textbf{Escrita Física do Bloco:} O exportador compila e anexa as propriedades visuais (coordenadas \lstinline|position|, o nó pai \lstinline|parent|, a dimensão \lstinline|size| e a cor \lstinline|color| no caso das Zonas) num objeto JSON serializado de linha única. Cada nó é escrito no ficheiro respeitando a assinatura padrão:
\begin{verbatim}
:: NomePassagem [tags] {"position":"X,Y", ...}
Conteúdo da passagem
\end{verbatim}
\end{enumerate}

\subsection{Gestão de Passagens Especiais}
O parser reconhece passagens reservadas com funções específicas na inicialização da história:
\begin{itemize}
...
\subsection{Modelo SugarCube (Preservação de Código e Indexação)}
No caso das macros de alteração de estado (ex: \lstinline|<<set $moedas += 10>>|) e condicionais lógicas (ex: \lstinline|<<if $tem_chave>> ... <</if>>|), o parser de importação não as compila nem as remove do texto. Em vez disso, o parser executa duas operações estruturais:
\begin{enumerate}
...
A interpolação matemática de Catmull-Rom foi programada diretamente no componente de forma a evitar dependências de pacotes externos, otimizando o desempenho e garantindo uma renderização leve no lado do cliente. Esta implementação foi adaptada a partir da solução documentada na comunidade de programadores do \textit{Stack Overflow} \citep{Aarts2018StackOverflow}. 
...
Para além do núcleo lógico e da simulação estática, a plataforma disponibiliza um assistente baseado em Modelos de Linguagem de Grande Escala (LLMs) para a importação rápida de histórias em formato de texto livre. A conceção e implementação desta funcionalidade assenta nos seguintes aspetos:

\subsection{Arquitetura de Integração Local}
A integração com inteligência artificial fornece uma interface de comunicação local e assíncrona entre o editor gráfico e o servidor de inferência do utilizador. O desenvolvimento focou-se na criação de um módulo de comunicação REST (\texttt{aiGenerator.js}) para estabelecer a ligação assíncrona com o endpoint local do Ollama (\texttt{http://localhost:11434}), enviando os parâmetros de temperatura e tokens via JSON. Em paralelo, desenhou-se um prompt de sistema especializado para forçar o modelo a gerar respostas estritamente em conformidade com a especificação Twee 3, mitigando alucinações. O fluxo de importação recebe o texto estruturado retornado pela API local, encaminha-o para o parser e gera automaticamente os nós e arestas correspondentes no canvas.

A decisão de basear este assistente em inferência local (via Ollama) garante a privacidade e soberania dos dados, assegurando que a propriedade intelectual da história e os textos criados permanecem confidenciais e livres de partilhas com servidores externos para treino de modelos comerciais. Adicionalmente, esta abordagem elimina custos financeiros de faturação associados a APIs proprietárias na nuvem, oferecendo uma utilização gratuita e ilimitada. Por questões de flexibilidade, o sistema permite também a configuração de chaves de API para serviços externos na nuvem, assegurando o acesso a utilizadores cujo hardware local seja limitado.

\subsection{Papel do LLM: Conversão e Importação Automática de Histórias}
A abordagem de design delimitou de forma clara o papel do modelo de linguagem no ciclo de vida da narrativa. Visto que os LLMs apresentam propensão a alucinações, criação de caminhos inacessíveis ou omissão de restrições de ciclos, o modelo não é integrado de forma dinâmica na simulação direta da história.

O papel do LLM restringe-se a uma ferramenta de conversão sintática em fase de pré-processamento. O modelo analisa a narrativa em linguagem natural fornecida pelo autor e traduz-a para a norma Twee 3. A partir desse ponto, o motor lógico da aplicação e o validador BFS assumem a responsabilidade exclusiva de renderizar o canvas visual e garantir a integridade de todos os caminhos e variáveis de estado no editor.