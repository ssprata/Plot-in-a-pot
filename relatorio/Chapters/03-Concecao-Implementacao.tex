...
\end{verbatim}
    ...
    \item \textbf{Lista de Adjacência Bidirecional (\texttt{adjacencyList}):} Representa o grafo direcionado de estados do ponto de vista do motor lógico. É recalculada de forma síncrona a cada modificação nos vetores de nós ou arestas. Esta lista mapeia cada identificador de cena ao seu conjunto de sucessores (saídas) e predecessores (entradas), permitindo ao validador e ao motor de simulação realizar percursos e pesquisas rápidas.
\end{enumerate}
...
O algoritmo de auditoria executa a exploração de acordo com os seguintes passos estruturados e detalhados:
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Modificação de Estado (\texttt{applyModifiers}):} O validador analisa o texto da cena corrente. Caso este texto contenha instruções de alteração de variáveis (como macros \texttt{<<set \$moedas += 10>>}), o motor interpreta estas instruções e atualiza o dicionário de variáveis local, gerando um novo estado de memória. Esta interpretação é efetuada por um analisador síncrono que converte a sintaxe SugarCube para expressões válidas de JavaScript.
...
Após o término da exploração (quando a fila de exploração fica vazia), o módulo de auditoria em \texttt{storyValidator.js} analisa os conjuntos acumulados para identificar e classificar as falhas de integridade narrativa no grafo:
\begin{itemize}
    ...
    \item \textbf{Escolhas Inacessíveis:} Escolhas cuja condição lógica de acessibilidade nunca pôde ser satisfeita sob nenhuma das combinações de variáveis e caminhos explorados durante o percurso (por exemplo, exigir uma chave que nunca é definida como obtida em nenhuma cena anterior).
\end{itemize}
...
Para suportar a avaliação de condições e a execução de modificadores de estado descritas no algoritmo de exploração, a plataforma integra um motor de compilação \textit{Just-In-Time} (JIT) simplificado, estruturado nos módulos \texttt{logicParser.js} e \texttt{sugarcubeLogic.js}. A arquitetura deste motor divide-se em quatro fases principais:
...
\subsection{Arquitetura de Integração e Papel do LLM}
A integração com modelos de linguagem fornece uma interface de comunicação assíncrona entre o editor gráfico e o servidor de inferência. O desenvolvimento focou-se na criação do módulo \texttt{aiGenerator.js}, um cliente REST que estabelece a ligação com o endpoint local do Ollama (\texttt{http://localhost:11434}) ou, alternativamente, com APIs na nuvem (como o Gemini) através de chaves externas, garantindo acessibilidade a utilizadores com limitações de hardware local. A inferência local, prioritária no sistema, assegura a soberania dos dados e a confidencialidade da propriedade intelectual, além de eliminar custos financeiros associados a serviços proprietários.

Para mitigar a propensão de alucinações, omissão de ciclos ou criação de caminhos inacessíveis por parte dos LLMs, o modelo não intervém na simulação direta ou no fluxo de execução do jogo. O seu papel restringe-se estritamente a uma ferramenta de conversão sintática em fase de pré-processamento. Através de um prompt de sistema especializado, o LLM analisa a narrativa em linguagem natural fornecida pelo autor e traduz-a para a norma Twee 3. O bloco de texto gerado é enviado para o interpretador da aplicação, que assume o controlo determinístico do grafo, restando ao validador BFS a responsabilidade exclusiva de auditar a integridade de todos os caminhos e variáveis de estado no canvas visual.