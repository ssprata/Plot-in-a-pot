...
Este apêndice constitui o Manual de Utilização e Instalação oficial do \textit{Plot in a Pot} (\gls{piap}). Destina-se a autores de ficção interativa, designers de jogos e programadores que pretendam instalar, configurar e explorar todas as capacidades lógicas, visuais e de tradução da plataforma.
...
\subsection{Importação e Exportação de Ficheiros CSV}
Para enviar os ficheiros a um tradutor externo profissional:
\begin{enumerate}
    \item Clique no botão ``Exportar CSV'' na Matriz de Tradução. Um ficheiro estruturado em formato tabular será descarregado.
    ...
    \item Após a tradução, clique em ``Importar CSV'' e selecione o ficheiro atualizado. O editor atualizará automaticamente todas as células e strings na memória.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Exportar Twee 3:} Clique em ``Exportar .twee'' para descarregar o ficheiro de texto estruturado. Este ficheiro preserva todos os dados posicionais das caixas e Zonas em metadados JSON compatíveis com outros compiladores de Twine.
